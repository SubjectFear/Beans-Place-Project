...
// Leafler map componet
function LocationMap() {
  const mapRef = useRef(null);
  // a box to store values that survives across renders - but changing it doesn't cause the component to re-render
  // null = is the current value. We say null beacuse there's no element yet when the component first runs
    ...
    <footer className="footer">
      <div className="mx-auto max-w-7xl px-6 pb-8 pt-16 sm:pt-24 lg:px-8 lg:pt-32 2xl:max-w-400">
        ...
        <div className="mt-16 grid grid-cols-2 gap-8 md:grid-cols-4">
          <ScrollReveal
            animation="fadeUp"
            className="col-span-2 md:col-1 space-y-4"
          >
            <img
            ...
            />
            <p className="footer-description justify-self-center md:place-self-auto">
            ...
            <div className="flex gap-x-6 justify-self-center md:justify-self-auto">
              {navigation.social.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="text-white/70 transition-colors duration-200 hover:text-(--amber)"
                >
        ...
        <div className="footer-bottom">
          <p>
            &copy; {currentYear} Ther Beans Place, LLC. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
